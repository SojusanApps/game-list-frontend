...
- For optimistic modals that close before the server answers (ADR 0008), "success" means the server confirmed — the draft is cleared from the mutation hook, and survives a failed save so reopening restores it.
